...
async function main() {
  ...
  console.log("FINAL_STATUS=PASS_ADMIN_RECONCILIATION_UI_RUNTIME_READY");
}
...
async function assertDetailPage(
  ...
  assertNoMutationForms(text, `${label} read-only forms`);
}
...
async function assertReadOnlyBoundary(runtime, adminJar, fixture) {
  for (const path of [
    ...
    assertNoMutationForms(text, `${path} read-only boundary`);
    assertHtmlPublicSafe(text, `${path} public safe`);
  }
...
function assertNoMutationForms(html, label) {
  ...
  for (const marker of MUTATION_LABEL_DENYLIST) {
    assert(
