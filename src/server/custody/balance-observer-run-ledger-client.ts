...
function validateScopeInput(input: RecordBalanceObserverScopeOutcomeInput): readonly unknown[] {
  ...
  if (new Set(failures.map((failure) => failure.bindingId)).size !== failures.length || new Set(failures.map((failure) => failure.bindingOrder)).size !== failures.length || validateBigint(input.bindingFailureCount) !== String(failures.length)) return invalidInput();
  ...
  return [validateUuid(input.runId), input.discoveryIndex, validateUuid(input.providerId), validateUuid(input.assetId), input.scopeStatus, validateBigint(input.bindingSuccessCount), String(failures.length), validateBigint(input.bindingAbortCount), refreshRequested, refreshAttempted, refreshSucceeded, refreshFailed, validateBigint(input.noLongerEligibleCount), validateCode(input.scopeCode, true), failures.map((f) => f.bindingId), failures.map((f) => f.bindingOrder), failures.map((f) => f.stage), failures.map((f) => f.code), failures.map((f) => f.retryable), failures.map((f) => f.adapterAttempts), failures.map((f) => f.databaseAttempts), failures.map((f) => f.retryExhausted), failures.map((f) => f.retryDeferred), failures.map((f) => f.requiresScopeRefresh)];
}
