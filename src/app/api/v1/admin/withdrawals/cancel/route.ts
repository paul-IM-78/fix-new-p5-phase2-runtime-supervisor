...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const execution = await adminCancelWithdrawalRequest(input);
...
function getCancelRedirectPath(result: AdminWithdrawalCommandResult): string {
  switch (result.resultCode) {
    case "APPLIED":
      return "/admin/withdrawals?result=withdrawal_request_canceled";
    case "NOOP":
      return "/admin/withdrawals?result=withdrawal_request_cancel_noop";
    ...
    case "WITHDRAWAL_REQUEST_NOT_CANCELABLE":
      return "/admin/withdrawals?error=withdrawal_request_not_cancelable";
