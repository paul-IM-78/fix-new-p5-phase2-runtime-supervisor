...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const execution = await approveWithdrawalRequest(input);

  if (!execution.ok) {
    return redirectNoStore(request, getErrorRedirectPath(execution.error));
  }

  return redirectNoStore(request, getApproveRedirectPath(execution.result));
}
...
function getApproveRedirectPath(
  result: AdminWithdrawalCommandResult,
): string {
  switch (result.resultCode) {
    case "APPLIED":
      return "/admin/withdrawals?result=withdrawal_request_approved";
    case "NOOP":
      return "/admin/withdrawals?result=withdrawal_request_approve_noop";
    ...
    case "WITHDRAWAL_REQUEST_NOT_APPROVABLE":
      return "/admin/withdrawals?error=withdrawal_request_not_approvable";
