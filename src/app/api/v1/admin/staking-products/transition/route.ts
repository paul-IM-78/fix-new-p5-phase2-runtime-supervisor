...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const newStatus = validateStakingProductStatus(
  ...
  return redirectNoStore(
    request,
    getTransitionRedirectPath(execution.result),
  );
}

function getTransitionRedirectPath(result: {
  resultCode: string;
  replayed: boolean;
}): string {
  if (result.replayed) {
  ...
  switch (result.resultCode) {
    case "APPLIED":
      return "/admin/staking-products?result=staking_product_status_changed";
    case "NOOP":
      return "/admin/staking-products?result=staking_product_status_noop";
    case "INVALID_INPUT":
    ...
    case "STAKING_PRODUCT_TRANSITION_INVALID":
      return "/admin/staking-products?error=staking_product_transition_invalid";
    case "STAKING_PROJECT_NOT_ACTIVE":
      return "/admin/staking-products?error=staking_project_not_active";
    case "STAKING_ASSET_NOT_ACTIVE":
      return "/admin/staking-products?error=staking_asset_not_active";
    case "STAKING_ASSET_NOT_PROJECT_TOKEN":
      return "/admin/staking-products?error=staking_asset_not_project_token";
    case "STAKING_ENROLLMENT_EXPIRED":
      return "/admin/staking-products?error=staking_enrollment_expired";
    default:
