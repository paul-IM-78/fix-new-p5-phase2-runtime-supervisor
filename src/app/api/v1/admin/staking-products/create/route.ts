...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const productCode = validateStakingProductCode(
...
function getCreateRedirectPath(result: {
  resultCode: string;
  replayed: boolean;
}): string {
  if (result.replayed) {
  ...
  switch (result.resultCode) {
    case "APPLIED":
      return "/admin/staking-products?result=staking_product_created";
    case "INVALID_INPUT":
    ...
    case "STAKING_PRODUCT_CODE_EXISTS":
      return "/admin/staking-products?error=staking_product_code_exists";
