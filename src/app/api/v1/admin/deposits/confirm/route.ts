...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const execution = await confirmDepositRequest({
    depositRequestId,
  ...
  return redirectNoStore(
    request,
    getConfirmRedirectPath(execution.result),
  );
}

function getConfirmRedirectPath(
  result: AdminDepositCommandResult,
): string {
  switch (result.resultCode) {
    case "APPLIED":
      return "/admin/deposits?result=deposit_request_confirmed";
    case "NOOP":
      return "/admin/deposits?result=deposit_request_confirm_noop";
    ...
    case "DEPOSIT_REQUEST_CANCELED":
      return "/admin/deposits?error=deposit_request_canceled_terminal";
    case "DEPOSIT_TARGET_PROFILE_NOT_ACTIVE":
      return "/admin/deposits?error=deposit_target_profile_not_active";
    case "DEPOSIT_TARGET_WALLET_NOT_ACTIVE":
      return "/admin/deposits?error=deposit_target_wallet_not_active";
    case "DEPOSIT_TARGET_ASSET_NOT_ACTIVE":
      return "/admin/deposits?error=deposit_target_asset_not_active";
